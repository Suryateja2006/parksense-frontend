...
const handlePaymentDone = async () => {
  ...
  const fetchQRCode = async (carNumber) => {
    try {
      ...
      const response = await fetch(
        `https://parksense-backend-x5a2.onrender.com/get-qr?plate_number=${encodeURIComponent(carNumber)}&t=${timestamp}`,
        {
          headers: {
  ...
  const handleQRScan = async () => {
    try {
      const response = await fetch("https://parksense-backend-x5a2.onrender.com/check-scan", {
        method: "POST",
      ...
      if (result.status === "released") {
        ...
        const statusResponse = await fetch(
          `https://parksense-backend-x5a2.onrender.com/slot-status?plate_number=${encodeURIComponent(userData.carNumber)}`,
        )
